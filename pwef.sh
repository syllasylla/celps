...
play_words_en_fr () {
	local lesson=$(ls -t $dir_cw_ef|gawk '{if(length($1)>0){print($1+1);exit;}else{print 1;}}')	
	...
	while true; do
		...
		if pwef_lesson_complete $flesson; then 
			return 0;
		fi
		pwef_show_words $fdict $flesson $lesson $ftest
		pwef_get_answers $flesson $frep

		if pwef_lesson_success $ftest $frep; then 
			cp $frep $dir_rw_ef"/$lesson"
			cp $flesson $dir_cw_ef"/$lesson"	
			echo -e "\t--- felicitation --";
